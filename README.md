...
equityUsd             1197.80
returnPct               19.78
realisedPnlUsd         197.80
feesUsd                  4.41
closedTrades             7.00
wins / losses         6 / 1     (85.7% win rate)
peakEquityUsd         1199.56
maxDrawdownPct          -2.72
```
...
pump-and-dump on the way down (−20.8%, exactly where the stop is set), and
never bought CATALYST, CONCAT, DOGWIFCAT, BONKAI, the honeypot or the
un-exitable pool. Those exclusions are asserted in `test/agent.test.js`.
...
**Honest status of this feature.** The anchoring code is complete and the pure
parts are unit-tested — record building, the hash chain, tamper detection, the
memo byte layout, the mainnet refusal and the "no default keypair" rule. What
is *not* yet demonstrated is a confirmed devnet transaction from this machine:
